...
export class Pagination extends Component {
    ...
    configureButtonNumberInfo() {
        ...
        if (curPage === 1) {
            ...
            if (pageIsLast) return [{ active: 'active',number: curPage}];
            return [{active: 'active', number: curPage}, {active: '',number: curPage + 1}];
        }
        //if curPage is last will be visible curPage-1 and curPage. For example: last page is 10, then it return 9, 10
        if (pageIsLast) {
            return [{active: '',number: curPage - 1}, {active: 'active',number: curPage}];
        }

        return [{active: '',number: curPage - 1}, {active: 'active',number: curPage}, {active: '',number: curPage + 1}];
    }
